...
function initSchema() {
  ...
  // Migration: add initialized column to existing databases that predate this version
  const cols = db.prepare('PRAGMA table_info(cash_balance)').all() as { name: string }[];
  if (!cols.some(c => c.name === 'initialized')) {
    db.exec('ALTER TABLE cash_balance ADD COLUMN initialized INTEGER DEFAULT 0');
  }
